...
export function swingTool(root, X) {
  ...
  <div class="stack">${groups.map(g => `<div class="row" style="gap:10px"><span style="flex:1">${icon(g.icon)} ${g.label}</span><input type="range" min="0" max="100" data-sw="${g.id}" value="${swings[g.id]}" style="flex:2"><span class="mono small" id="swv_${g.id}" style="width:36px;text-align:right">${swings[g.id]}</span><span class="mono small muted" id="sww_${g.id}" style="width:56px;text-align:right"></span></div>`).join('')}</div>
...
export function ahpTool(root, X) {
  ...
  const compute = () => {
    ...
    $('#ahpOut', root).innerHTML = `<div class="row between"><span class="small"><b>CR = ${r.cr.toFixed(3)}</b> <span class="pill ${r.consistent ? 'good' : 'bad'}">${r.consistent ? 'consistent' : 'inconsistent'}</span></span><span class="small muted">λmax ${r.lambdaMax.toFixed(3)} · n ${n}</span></div>
      <div class="stack" style="margin-top:6px">${groups.map((g, i) => `<div class="row between small"><span>${icon(g.icon)} ${g.label}</span><span class="mono">${(r.weights[i] * 100).toFixed(1)} %</span></div>`).join('')}</div>
